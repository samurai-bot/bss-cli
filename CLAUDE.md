...
- **(v0.17+) Don't overload the `Case` aggregate for port requests.** Port requests are their own aggregate (`crm.port_request`) with their own FSM (`requested → validated → completed | rejected`) and their own audit-event family (`port_request.*`). Cases are customer-touched escalations; port requests are operational state machines that the operator drives. Mixing them muddles the audit trail and blocks the (post-v0.17) path to a real TMF629/TMF641 customer-port handoff.
- **(v0.17+) Don't release a ported-out MSISDN back to `available`.** Port-out is a *terminal* status (`ported_out`) on `inventory.msisdn_pool`, not a release. The number belongs to the recipient carrier; recycling it inside our pool would re-issue a number we've handed away. `quarantine_until = '9999-12-31'` is the marker so the existing reserve-next predicate (`status='available'`) can never select it. Greppable: `rg "set status\s*=\s*'available'.*ported_out|ported_out.*'available'" services/crm/app/repositories/msisdn_repo.py` must stay empty.
- **(v0.17+) Don't add a new `event_type` for roaming.** Roaming is a per-event *attribute* (`roaming_indicator: bool`), not an event-type discriminator. The mediation `VALID_EVENT_TYPES` set stays at `{data, voice, voice_minutes, sms}`. Rating decides routing by inspecting the boolean post-rate. A `data_roaming` event type would force every existing usage poster to learn about roaming and break v0.10–v0.16 scenarios.
- **(v0.17+) Don't cascade roaming-balance exhaustion into home-data exhaustion.** Block-on-exhaust applies to the *roaming* balance independently — if `data_roaming` is 0, roaming usage is rejected with rule `subscription.usage_rated.roaming_balance_required`, but the subscription stays `active` (home data still works). The `is_exhausted` predicate in `app.domain.bundle` must continue to consider only the *primary* allowance set; `data_roaming` is *additive*, not blocking.
- **(v0.17+) Don't expose port-request writes to the `customer_self_serve` profile.** MNP is operator-driven by spec (donor-carrier coordination, fraud-screen, regulatory clearance). Profile entries live in `operator_cockpit` only; `validate_profiles()` self-checks at startup. The cockpit operator IS the trust principal here.
- **(v0.18+) Don't trigger renewal from anywhere except the subscription lifespan tick loop.** The renewal worker lives in `services/subscription/app/workers/renewal.py` and is started in `services/subscription/app/dependencies.py:lifespan`. No sibling scheduler container, no cron entry, no Celery beat, no external poller. The whole point of `FOR UPDATE SKIP LOCKED` in the sweep query is "in-process tick in every replica" multi-replica safe by construction; running the trigger elsewhere defeats the design and creates double-billing risk. Manual `subscription.renew_now` (CLI / cockpit / scenario action) stays as the operator escape hatch only — the automatic dispatch is the sole production path. Greppable + grep guard: `rg '(_renewal_tick_loop|_sweep_due|_sweep_skipped)' --type py` outside the worker module + dependencies.py + tests must stay empty.
- **(v0.18+) Don't duplicate `subscription_service.renew()` logic in the worker.** The worker calls `service.renew(sub_id)` and nothing else. No re-implementation of charge, no parallel state machine, no inline pivot of `pending_offering_id`. If renewal grows a new step (notification, additional audit emission, side effect), it goes into `renew()` once and the worker picks it up for free. The worker's job is *triggering*, not *renewing*. Greppable: `services/subscription/app/workers/renewal.py` must contain at most one `service.renew(` call site per sweep, and zero `payment_client.charge`, `add_state_history`, or `_transition` calls.
- **(v0.20+) Don't index `phases/V0_*.md` in the knowledge tool.** Phase docs are historical build plans — they describe what was *planned* at a point in time, not what *is*. Indexing them lets the LLM cite a deferred feature as if shipped, or quote a draft env var name that was renamed before merge. Index `CLAUDE.md`, `docs/HANDBOOK.md`, `docs/runbooks/*.md`, `ARCHITECTURE.md`, `DECISIONS.md`, `TOOL_SURFACE.md`, `ROADMAP.md`, `CONTRIBUTING.md` — and that's it. The allowlist in `bss_knowledge.paths.INDEXED_PATHS` is the source of truth; adding a new path requires a doctrine review. Doctrine guard 16 in `make doctrine-check` enforces.
- **(v0.20+) Don't expose `knowledge.search` / `knowledge.get` in `customer_self_serve`.** The handbook + runbooks describe destructive operator flows, perimeter posture, named-token rotation, KYC bypass flags, the cutover runbook for Stripe. None of that belongs in the customer chat — leaking it would teach a prompt-injected LLM exactly which flag to ask the operator about. The greppable rule: `knowledge.search` and `knowledge.get` appear in `TOOL_PROFILES["operator_cockpit"]` only; `validate_profiles()` enforces. Doctrine guard 15 in `make doctrine-check` catches any literal leakage in source.
- **(v0.20+) Don't paraphrase doctrine without citing.** When the operator asks a how-to / what-is / is-this-allowed question, the cockpit's contract is `knowledge.search` → cite anchor, OR say "I don't have a citation for that." The `_RE_KNOWLEDGE_CLAIM` guard at the REPL + browser cockpit replaces un-cited handbook claims with a templated fallback pointing at `bss admin knowledge search`. A model that reliably trips the guard is a search-relevance bug or a prompt regression — fix the search index or the system prompt, don't relax the regex.
