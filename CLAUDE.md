...
- **(v0.4–v0.10 / signup + chat) Don't write to bss-clients from a portal route handler on these surfaces.** The signup funnel and the chat surface go through the orchestrator via `agent_bridge.*` → `astream_once`. Reads can go direct.
...
- **(v0.11+ committed, see DECISIONS 2026-04-27 + `phases/V0_11_0.md`)** The signup funnel migrates to the direct-API path post-v0.10. v0.11 ships the migration; once tagged, only the chat surface is orchestrator-mediated. The v0.4 agent-log demo artifact is retired from the primary signup path; the educational story moves to the LLM heroes (`llm_troubleshoot_blocked_subscription`, `portal_csr_blocked_diagnosis`) and the chat surface. The greppable doctrine guard updates accordingly: `rg 'astream_once' portals/self-serve/bss_self_serve/routes/` must match only the chat route after v0.11 tags.
